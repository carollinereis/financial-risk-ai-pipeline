...
* Cause: Running Python 3.8 or 3.9 with union syntax (dict | None), introduced in Python 3.10.

* Solution: Import `Optional` from `typing:`
...
### Issue 2: `TypeError: SecuritySanitizer() takes no arguments`

* Cause: Passing parameters to `SecuritySanitizer()` when `__init__` lacks default argument handling.

* Solution: Bind `ENABLE_PII_MASKING` from `config.py` inside `__init__`:
...
* Cause: Executing `database.py` or `security.py` prior to dataset creation.
...
./venv/bin/python src/security.py
```